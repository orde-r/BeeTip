...
The server exposes only `GET`, `POST`, and `OPTIONS` through CORS.

## Authentication Model

Protected REST endpoints accept either:

- `Authorization: Bearer <accessToken>`
- `accessToken` cookie

For the frontend SPA, prefer the bearer token returned by `POST /auth/login`. The login route also sets an HttpOnly `accessToken` cookie with `secure: true` and `sameSite: Strict`, but the current CORS setup does not enable credentialed cross-origin requests, so bearer auth is the more predictable frontend path.

Socket.io auth also requires the same token:
...
- There is no backend route yet to toggle or persist Buyer/Kurir mode. Treat the frontend role toggle as UI state unless a backend route is added.
...
| `POST` | `/auth/logout` | No in route, token optional in frontend | Clear auth cookie |
...
- Store `accessToken` in the existing auth state.
- Attach it as `Authorization: Bearer <token>` on protected requests.
- Use the same token for Socket.io.
...
Returns the authenticated user. Use this after login, after deposit, after payment, after completion, or on app bootstrap when a token exists.
...
- `401` missing, invalid, expired token, or deleted user session

### `POST /auth/logout`

Clears the `accessToken` cookie and returns a success message.

Auth: route is public in the backend. The frontend may still send the bearer token for consistency.
...
- Clear local access token and auth state regardless of cookie behavior.
...
- In Kurir mode, show these as accept-able order cards.
...
Alternative accepted by backend:
