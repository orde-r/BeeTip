...
export const TopUpBodySchema = z
  .object({
    amount: z.number().positive("Amount must be positive").openapi({ example: 100000.0 }),
  })
  .openapi("TopUpBody");
...
export const TransactionDTOSchema = z
  .object({
    ...
    amount: z.number().openapi({ example: 100000.0 }),
...
export const TopUpResponseSchema = z
  .object({
    ...
    new_balance: z.number().openapi({ example: 100000.0 }),
  })
